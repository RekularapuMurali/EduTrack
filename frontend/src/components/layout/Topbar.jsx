          ...
          {notifOpen && (
            <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', width: 300, background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 14, boxShadow: '0 8px 24px rgba(0,0,0,0.10)', zIndex: 50, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {unreadCount > 0 && <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 999, background: '#DCFCE7', color: '#166534' }}>{unreadCount} new</span>}
              </div>
              {NOTIFICATIONS.map((n, i) => (
                ...
                >
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: n.unread ? '#22C55E' : 'transparent', flexShrink: 0, marginTop: 4 }} />
                  <div>
              ...
              <div style={{ padding: '10px 16px', borderTop: '1px solid #F1F5F9', textAlign: 'center' }}>
                <button style={{ fontSize: 12, color: '#166534', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer' }}>
