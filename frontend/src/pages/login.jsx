...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    login({ ...account, email }, 'demo-jwt-token');
    setLoading(false);
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      ...
      {/* Left green panel */}
      <div style={{ width: '42%', background: 'linear-gradient(160deg, #166534 0%, #15803D 100%)', padding: 40, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div>
          ...
          {[['1,200+', 'Students Tracked'], ['85', 'Active Volunteers'], ['4,500+', 'Sessions Completed']].map(([val, label]) => (
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, background: '#F8FAFC' }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <p style={{ fontSize: 13, color: '#64748B', marginBottom: 28 }}>Sign in to your dashboard</p>

          {/* Role switcher */}
          <div style={{ background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: 12, padding: 14, marginBottom: 24 }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
          ...
          <form onSubmit={handleSubmit}>
            {error && (
              <div style={{ padding: '10px 14px', borderRadius: 8, background: '#FEE2E2', color: '#991B1B', fontSize: 13, marginBottom: 14, border: '1px solid #FECACA' }}>
                {error}
              </div>
            )}

            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: '#374151', display: 'block', marginBottom: 4 }}>Email <span style={{ color: '#EF4444' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: '#374151', display: 'block', marginBottom: 4 }}>Password <span style={{ color: '#EF4444' }}>*</span></label>
