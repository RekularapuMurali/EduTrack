      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ padding: '0 20px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14, marginTop: -28, marginBottom: 14 }}>
            <div style={{ width: 56, height: 56, borderRadius: 14, background: '#166534', border: '3px solid #FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: 'white', flexShrink: 0 }}>
              {user?.avatar || user?.name?.slice(0,2).toUpperCase() || 'AS'}
            </div>
            <div style={{ paddingBottom: 4 }}>
              <h2 style={{ fontSize: 17, fontWeight: 700, color: '#0F172A', margin: 0 }}>{user?.name || 'Student'}</h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          {nextLevel && (
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: 12, color: '#64748B', margin: 0 }}>Next: <span style={{ color: '#166534', fontWeight: 600 }}>{nextLevel.icon} {nextLevel.name}</span></p>
          ...
          <Card title="My Academic Progress" subtitle="Score trend">
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={trendData}>
                ...
                <Line type="monotone" dataKey="score" stroke="#166534" strokeWidth={2.5} dot={{ r: 5, fill: '#166534', strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {activities.slice(0, 4).map(a => (
                <div key={a._id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#F8FAFC', borderRadius: 10, padding: 12 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: 14, fontWeight: 700, color: '#166534', margin: 0 }}>+{a.pointsEarned}</p>
